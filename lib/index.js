"use strict";
var clean, config, install, logger, path, registerCommand, registration, watcher, _callIfModuleIncluded, _debug, _prepBowerClean, _prepBowerInstall, _prepBowerLibraryInstall, _watch;
...
registerCommand = function(program, retrieveConfig) {
  ...
  program.command('bower:install <names>').option("-D, --debug", "run in debug mode").option("-d, --savedev", "save to dev dependencies instead of dependencies").description("Install a library and update the bower.json accordingly").action(function(names, opts) {
    names = names.split(',');
