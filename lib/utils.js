"use strict";
var bower, fs, logger, path, strategy, wrench, _addResolvedPath, _handlePackageJson, _isPathExcluded, _resolvePaths;
...
_resolvePaths = function(mimosaConfig, names, paths) {
  ...
  for (lib in installedPaths) {
    ...
    if (mimosaConfig.bower.copy.overridesArrays[lib]) {
      mimosaConfig.bower.copy.overridesArrays[lib].forEach(function(override) {
        ...
        if (fs.existsSync(overridePath)) {
          ...
          if (pathStat.isDirectory()) {
            return wrench.readdirSyncRecursive(overridePath).map(function(filePath) {
              ...
              return _addResolvedPath(mimosaConfig, resolvedPaths[lib], filePath);
            });
          } else {
