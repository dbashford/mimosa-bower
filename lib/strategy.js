...
transforms.packageRoot = function(mimosaConfig, inPath, lib) {
  ...
  fileName = path.basename(inPath);
  if (_isJavaScript(inPath)) {
    return path.join(mimosaConfig.vendor.javascripts, mimosaConfig.bower.copy.outRoot, mimosaConfig.bower.copy.outRoot, lib, fileName);
  } else {
...
transforms.custom = function(mimosaConfig, lib, inPath) {
  ...
  if (_isJavaScript(modInPath)) {
    return path.join(path.join(mimosaConfig.vendor.javascripts, mimosaConfig.bower.copy.outRoot), modInPath);
  } else {
    return path.join(path.join(mimosaConfig.vendor.stylesheets, mimosaConfig.bower.copy.outRoot), modInPath);
  }
};
