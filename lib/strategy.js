...
transforms.none = function(mimosaConfig, inPath, root) {
  var modInPath;
  if (root == null) {
    root = '';
  }
  ...
  if (_isJavaScript(modInPath)) {
    return modInPath.replace(mimosaConfig.bower.bowerDir.pathFull, path.join(mimosaConfig.vendor.javascripts, root));
  } else {
    return modInPath.replace(mimosaConfig.bower.bowerDir.pathFull, path.join(mimosaConfig.vendor.stylesheets, root));
  }
};

transforms.custom = function(mimosaConfig, lib, inPath, root) {
  var matchedOverride, modInPath, overrideObject;
  if (root == null) {
    root = '';
  }
  modInPath = _replacePathPieces(mimosaConfig, inPath);
  ...
  if (_isJavaScript(modInPath)) {
    return path.join(path.join(mimosaConfig.vendor.javascripts, root), modInPath);
  } else {
    return path.join(path.join(mimosaConfig.vendor.stylesheets, root), modInPath);
  }
};
